...
INSERT INTO deal_statuses (code, title, sort_order) VALUES
  ...
  ('RISK_REVIEW',       'Проверка риска',               6),
  ('FINANCE_REVIEW',    'Финансовое утверждение',       7),
  ('INVESTOR_PENDING',  'Одобрение инвестора',          8),
  ('CONTRACT_PREP',     'Подготовка договора',          9),
  ('DOC_SIGNING',       'Подписание документов',       10),
  ('SIGNING_FUNDING',   'Подписание и финансирование', 11),
  ('VEHICLE_DELIVERY',  'Выдача автомобиля',           12),
  ('ACTIVE',            'Активный лизинг',             13),
  ('CANCELLED',         'Отменена',                    14)
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO deal_transitions (from_status, to_status, role_required) VALUES
  ...
  ('VEHICLE_CHECK',    'DOCS_COLLECT_BUYER',     'TECH_SPECIALIST'),
  ('DOCS_COLLECT_BUYER',     'DOCS_COLLECT_SELLER', 'OP_MANAGER'),
  ('DOCS_COLLECT_SELLER', 'RISK_REVIEW',    'OP_MANAGER'),
  ...
  ('DOCS_COLLECT_BUYER',     'CANCELLED',        'OP_MANAGER'),
  ('DOCS_COLLECT_SELLER', 'CANCELLED',      'OP_MANAGER'),
