...
- **Покупатель** — активный лизингополучатель с действующим договором.
...
| Модуль | Гость | Заявитель | Покупатель | Оп. специалист | Оп. менеджер | Менеджер CRM | Финансы | Кладовщик | Сервис-коорд. | Админ | Инвестор | Партнер |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| 7.0 Настройки | - | - | - | - | - | - | - | - | - | V/E | - | - |
| 8.0 Кабинет покупателя | - | V | V/E | - | - | - | V (агрег.) | - | - | V (имперсон.) | - | - |
...
- **URL (Path):** /
- **Роли доступа:** Гость (просмотр), Покупатель (просмотр), Инвестор (просмотр)
...
- **Основные переходы (Связи):** На `/catalog`, `/apply`, `/`, `/contact`.
...
- **Основные переходы (Связи):** На `/apply/start`, `/contact`, `/`.
...
- **Основные переходы (Связи):** На `/offer`, `/` (если уже есть кабинет).
...
- **Основные переходы (Связи):** На `/`, на `/apply/documents` (дозагрузка), чат `/support/new`.
...
- **Ключевые блоки и компоненты:** Форма логина, выбор MFA, кнопки "Забыли пароль", разделение потоков (покупатель/инвестор/партнер).
...
- **Основные переходы (Связи):** На `/password-reset/verify`, `/`.
...
- **Назначение:** Быстрый поиск по покупателям, сделкам, авто, задачам, документам.
...
## 3.0 Модуль CRM (Покупатели и Сделки)
### 3.1 Список лидов
...
### 3.4 Список покупателей (B2C)
- **Название:** Список покупателей
...
- **Назначение:** Каталог всех покупателей с фильтрами по статусам, договорам и рискам.
- **Ключевые блоки и компоненты:** Таблица покупателей, теги риска, фильтр активных договоров, экспорт.
- **Основные переходы (Связи):** На `/crm/clients/:id`, `/crm/segments`, `/support/tickets?clientId=`.

### 3.5 Карточка покупателя (внутренняя)
- **Название:** Карточка покупателя (внутренняя)
...
- **Назначение:** Полное досье покупателя, договоры, платежи, взаимодействия.
...
#### 3.5.1 Модал "Редактировать данные покупателя"
- **Название:** Редактировать данные покупателя
...
- **Назначение:** Обновление контактных и юридических данных покупателя.
...
- **Ключевые блоки и компоненты:** Детали контакта, привязанные покупатели, история коммуникаций.
...
- **Назначение:** Управление всеми документами покупателей, статусы проверки и интеграция с DMS.
...
- **Ключевые блоки и компоненты:** Основные характеристики, фото, текущий покупатель, график, телематика, сервисная история, повреждения.
...
#### 4.3.2 Модал "Назначить покупателю"
- **Название:** Назначить автомобиль покупателю
- **URL (Path):** /fleet/vehicle/:id#assign-client
- **Роли доступа:** Операционный менеджер (редактирование), Менеджер CRM (редактирование)
- **Назначение:** Закрепление авто за договором покупателя.
...
- **Роли доступа:** Финансовый менеджер (просмотр/редактирование), Операционный менеджер (просмотр), Администратор системы (просмотр), Инвестор (просмотр агрегатов в режиме read-only), Покупатель (просмотр агрегатов через имперсонацию)
...
- **Роли доступа:** Финансовый менеджер (просмотр/редактирование), Операционный специалист (просмотр), Операционный менеджер (просмотр), Покупатель (через имперсонацию read-only)
...
- **Роли доступа:** Финансовый менеджер (просмотр/редактирование), Менеджер CRM (просмотр), Покупатель (просмотр), Операционный менеджер (просмотр)
...
## 8.0 Внешний кабинет покупателя
### 8.1 Покупательский дашборд
- **Название:** Покупательский дашборд
- **URL (Path):** /client
- **Роли доступа:** Покупатель (просмотр), Операционный специалист (имперсонация-посмотр), Операционный менеджер (имперсонация-посмотр), Администратор системы (имперсонация)
- **Назначение:** Основной обзор статуса договора, платежей и сервисов для покупателя.
...
- **Роли доступа:** Покупатель (просмотр), Операционный специалист (имперсонация), Финансовый менеджер (просмотр)
...
- **Роли доступа:** Покупатель (редактирование), Операционный специалист (поддержка)
- **Назначение:** Подписание обновлений договора, допсоглашений.
- **Ключевые блоки и компоненты:** Просмотр PDF, поле подписи, парольный вход (email + пароль).
- **Основные переходы (Связи):** Возврат в `/client/agreements/:id`.

### 8.3 Центр платежей
- **Название:** Центр платежей покупателя
- **URL (Path):** /client/payments
- **Роли доступа:** Покупатель (просмотр/оплата), Финансовый менеджер (имперсонация-посмотр)
...
- **Роли доступа:** Покупатель (просмотр), Финансовый менеджер (просмотр)
...
- **Роли доступа:** Покупатель (просмотр), Сервис-координатор (имперсонация)
- **Назначение:** Показ текущего пробега, статуса сервисов, телематика для покупателя.
...
- **Название:** Сервисные запросы покупателя
- **URL (Path):** /client/service-requests
- **Роли доступа:** Покупатель (просмотр/создание), Сервис-координатор (имперсонация), Партнер (просмотр назначенных)
...
- **Роли доступа:** Покупатель (просмотр/переписка), Сервис-координатор (имперсонация), Партнер (просмотр/обновление статуса)
...
- **Название:** Поддержка покупателя
- **URL (Path):** /client/support
- **Роли доступа:** Покупатель (просмотр/создание), Операционный специалист (обработка), Операционный менеджер (контроль)
...
### 8.9 Документы покупателя
- **Название:** Документы покупателя
- **URL (Path):** /client/documents
- **Роли доступа:** Покупатель (просмотр/загрузка), Операционный специалист (имперсонация), Администратор системы (имперсонация)
...
- **Название:** Реферальная программа покупателя
- **URL (Path):** /client/referrals
- **Роли доступа:** Покупатель (просмотр/приглашение)
...
### 8.11 Настройки профиля покупателя
- **Название:** Настройки покупателя
- **URL (Path):** /client/profile
- **Роли доступа:** Покупатель (просмотр/редактирование)
...
- **Роли доступа:** Покупатель (редактирование)
...
- **Назначение:** Обработка тикетов покупателей, инвесторов, партнеров.
