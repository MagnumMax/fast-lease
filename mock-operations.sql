...
SELECT 
    ...
    END,
    CASE 
        WHEN RANDOM() > 0.7 THEN 'Назначена новая задача по договору DEAL-2025-' || LPAD((ROW_NUMBER() OVER())::text, 4, '0') || '. Требуется проверка документов клиента в течение 24 часов.'
        ...
        ELSE 'Сделка DEAL-2025-' || LPAD((ROW_NUMBER() OVER())::text, 4, '0') || ' требует внимания руководства. Обратитесь к операционному менеджеру.'
    END,
    'task',
    'warning',
...
SELECT 
    ...
    END,
    CASE 
        WHEN RANDOM() > 0.6 THEN 'Выявлен просроченный платеж по договору DEAL-2025-' || LPAD((ROW_NUMBER() OVER())::text, 4, '0') || '. Сумма: ' || (RANDOM() * 50000 + 10000)::numeric(16,2) || ' AED. Требуется уведомление клиента.'
...
SELECT 
    ...
    CASE 
        ...
        WHEN RANDOM() > 0.3 THEN 'Завершено техническое обслуживание автомобиля по договору DEAL-2025-' || LPAD((ROW_NUMBER() OVER())::text, 4, '0') || '. Все работы выполнены в срок. Автомобиль готов к выдаче.'
