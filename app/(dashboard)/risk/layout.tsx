...
import { resolveProfileHrefForRole, riskNav } from "@/lib/navigation";
...
export default async function RiskLayout({
  ...
  const sessionUser = await requirePortalSession("app", "/risk/dashboard");
  ...
  const navItems = await filterNavItemsForRoles(riskNav, sessionUser.roles);

  return (
    <DashboardLayout
      navItems={navItems}
      brand={{ title: "Risk", subtitle: "Fast Lease" }}
      user={{
        fullName,
        email,
