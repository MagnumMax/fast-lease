...
### 3. Модуль заявок
- **Заявки покупателей**: процесс подачи и рассмотрения заявлений
...
### 6. Покупательский портал
- **Поддержка**: система тикетов и общения
- **Уведомления**: персонализированные оповещения
- **Реферальная программа**: отслеживание привлеченных покупателей
...
| **Поддержка** | SUPPORT | Покупательская поддержка | `/support/dashboard` |
...
| **Покупатель** | CLIENT | Личный кабинет покупателя | `/client/dashboard` |
...
Сервисная функция под `security definer`, дающая возможность безопасно получать запись из `auth.users` по email без обращения к нестабильной пагинации GoTrue. Выполняется в `auth`-схеме и доступна только ролям `service_role` и `supabase_auth_admin`.
...
```mermaid
graph TD
    A[Покупатель заходит на сайт] --> B[Выбор автомобиля]
...
5. **Покупательский сервис**: система поддержки и реферальная программа
