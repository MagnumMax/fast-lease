...
begin
  for vehicle_row in (
    select *
    from (values
      ('WDCPEAK00000001', 'Rolls-Royce', 'Cullinan', 'Black Badge', 2024, 'SUV', 'petrol', 'automatic', 1200, 1100000, 1050000, 1020000, 'available', jsonb_build_object('color','Obsidian','package','Executive')),
...
begin
  ...
  for app_record in (
    ...
    values (
      ...
      jsonb_build_object(
        'provider','AXA',
        'policy_number',format('AXA-%06s', app_record.rn),
        'policy_type','comprehensive',
        'premium_amount', (4800 + app_record.rn * 120) * 0.4,
        'payment_frequency','monthly',
        'next_payment_due', to_char(current_date + interval '30 days','YYYY-MM-DD'),
        'coverage_start', to_char(current_date - interval '10 days','YYYY-MM-DD'),
        'coverage_end', to_char(current_date + interval '12 months','YYYY-MM-DD'),
        'deductible', 500,
