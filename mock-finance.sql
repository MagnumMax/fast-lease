...
SELECT 
    ...
    'insurance_premiums',
    'Страховые премии по полисам покупателей',
    '7 дней',
    (SELECT user_id FROM profiles WHERE full_name = 'Абдулла Аль-Накхи' LIMIT 1),
    '2024-12-01 09:00:00+04'::timestamptz,
    CURRENT_TIMESTAMP
...
SELECT 
    ...
    END,
    CASE 
        WHEN RANDOM() > 0.8 THEN 'Покупатель игнорирует требования об оплате. Рекомендуется юридическое взыскание.'
        WHEN RANDOM() > 0.6 THEN 'Последнее уведомление отправлено. Ожидается ответ в течение 3 дней.'
        WHEN RANDOM() > 0.4 THEN 'Покупатель просит рассрочку платежа на 3 месяца.'
...
(
    ...
    'high_risk',
    '2024-12-01'::date,
    'Покупатель имеет историю просрочек платежей. Текущая задолженность составляет 3 месяца.',
    'active',
    (SELECT user_id FROM profiles WHERE full_name = 'Султан Аль-Муваллад' LIMIT 1),
...
(
    ...
    'medium_risk',
    '2024-12-01'::date,
    'Недавние финансовые затруднения покупателя. Снижение кредитного рейтинга.',
    'active',
    (SELECT user_id FROM profiles WHERE full_name = 'Султан Аль-Муваллад' LIMIT 1),
    '2024-12-01 10:00:00+04'::timestamptz,
    (SELECT user_id FROM profiles WHERE full_name = 'Абдулла Аль-Накхи' LIMIT 1),
...
SELECT 
    ...
    'AED',
    CASE 
        WHEN RANDOM() > 0.8 THEN 'Поступления от лизинговых платежей покупателей'
...
SELECT 
    ...
    CASE 
        ...
        WHEN RANDOM() > 0.4 THEN 'Дополнительные требования покупателя'
...
-- 10.3 Показатели рентабельности по покупателям
INSERT INTO financial_kpis (
    id, kpi_type, kpi_name, calculation_method, target_value, actual_value,
    variance_percent, measurement_period, department, calculated_by, calculated_at
) VALUES
...
-- Связываем финансовые данные с существующими сделками и покупателями
...
-- Добавляем финансовую информацию в профили покупателей
UPDATE profiles 
...
-- Связность: Все данные корректно связаны с существующими сделками и покупателями
